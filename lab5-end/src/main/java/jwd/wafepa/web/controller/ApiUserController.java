...
public class ApiUserController {
	...
	ResponseEntity<List<UserDTO>> getUser(@RequestParam(value="page", defaultValue = "0" ) int page){
		Page<User> users = userService.findAll(page);
		
		if(users == null || page > users.getTotalPages()){
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}
		
		return new ResponseEntity<>(toDto.convert(users.getContent()),
				HttpStatus.OK);
	}
